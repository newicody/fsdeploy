...
| **7.9**   | Nettoyer et centraliser `contrib/` dans `fsdeploy/contrib/` (suppression des doublons sysvinit/upstart). | ✅          | Dossier `contrib/` nettoyé, fichiers OpenRC/systemd centralisés. |
...
| **Étape** | **Problème**                                                     | **Tâche**                                                                                       | **Fichiers**                                                                         | **Statut**               |
| --------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------ |
| **7.10**  | Dossier `lib/ui/` à la racine redondant.                         | **Supprimer `lib/ui/**` (fichier `mixins.py` inutile).                                          | `lib/ui/mixins.py`                                                                   | ⏳ **À faire maintenant** |
| **7.11**  | `Scheduler` n’a pas de `global_instance()`.                      | **Ajouter `global_instance()**` à `fsdeploy/lib/scheduler/core/scheduler.py`.                   | `fsdeploy/lib/scheduler/core/scheduler.py`                                           | ⏳                        |
| **7.12**  | `__main__.py` ne passe pas `runtime` → `self.bridge = None`.     | **Initialiser `Runtime` et `FsDeployConfig**` dans `__main__.py` et les passer à `FsDeployApp`. | `fsdeploy/__main__.py`                                                               | ⏳                        |

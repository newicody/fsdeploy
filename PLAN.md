...
Objectif : Résoudre les incohérences de structure, les bugs d’initialisation, et les duplications dans le projet.
✅ 7.0–7.7 : Tâches Déjà Terminées (Vérifiées)

    7.0 launch.sh : Branche dev par défaut + options --run/--no-run.
    7.1 live/setup.py : Correction de l’initialisation des linux-headers via uname -r.
    7.2 Sync des écrans dans tests/ (stale copies) → Fichiers corrigés.
    7.4 README.md : Mise à jour des instructions d’installation pour dev.
    7.5 DIAGRAMS.md : Correction des chemins linux-headers.
    7.6 fsdeploy_main_status.md : Suppression (obsolète).
    7.7 fsdeploy/lib/function/module/registry.py : Re-export corrigé.

🔴 7.8–7.16 : Problèmes et Tâches à Résoudre (Priorité Absolue)
📌 7.8 : Éliminer la duplication entre tests/fsdeploy/ et fsdeploy/

Problème :

    Le dossier tests/fsdeploy/ est une copie partielle et obsolète de fsdeploy/ (29 fichiers).
    Contient des stubs inutiles (cross_compile_screen.py, moduleregistry_screen.py) qui re-exportent depuis des fichiers existants dans fsdeploy/lib/ui/screens/.
    Incohérences : Certains fichiers dans tests/fsdeploy/ sont plus complets que dans fsdeploy/ (ex: crosscompile.py).
...
Problème :
    ...
    contrib/ à la racine contient des scripts sysvinit/upstart obsolètes.
    fsdeploy/contrib/ contient des scripts OpenRC/systemd modernes.
    Redondance : Les fichiers dans contrib/sysvinit/ et contrib/upstart/ font doublon avec ceux dans fsdeploy/contrib/openrc/ et fsdeploy/contrib/systemd/.
...
Problème :
    ...
    Le dossier lib/ui/ à la racine contient un seul fichier (mixins.py) redondant.
    BridgeScreenMixin est déjà implémenté dans fsdeploy/lib/ui/.
    Aucun fichier dans fsdeploy/ n’importe depuis lib.ui.*.

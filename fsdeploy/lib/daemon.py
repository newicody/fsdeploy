...
class FsDeployDaemon:
    ...
    def _connect_message_bus(self) -> None:
        """Connecte le bus global de messages à la file d'événements du scheduler."""
        if self._runtime and hasattr(self._runtime, 'event_queue'):
            set_event_queue(self._runtime.event_queue)
            log.info("message_bus_connected")

...
ATTENTION : les callbacks de montage utilisent _on_mount_done (pas _on_mount)
pour eviter toute collision avec le lifecycle Textual on_mount().
"""
import json
import os
...
CHECK, CROSS, WARN, ARROW = (("[OK]", "[!!]", "[??]", "->") if IS_FB
                              else ("✅", "❌", "⚠️", "→"))
...
class MountsScreen(Screen):
    ...
    def on_mount(self) -> None:
        """Lifecycle Textual — aucun argument."""
    ...
    def action_umount_selected(self):
        ...
        self.bridge.emit(
            "mount.umount",
            dataset=e["dataset"],
            mountpoint=e.get("current", ""),
            callback=lambda t, d=e["dataset"]: self._on_umount_done(t, d),
        )
        ...
        for e in self._entries:
            if e["mounted"]:
                ...
                self.bridge.emit(
                    ...
                    callback=lambda t, d=ds: self._on_verify_done(t, d),
                )
    ...
    def _mount_one(self, entry):
        ...
        self.bridge.emit(
            ...
            callback=lambda t, d=ds: self._on_mount_done(t, d),
        )
        self._log(f"  -> mount.request({ds} {ARROW} {mp})")

    # ── Callbacks (noms _on_*_done pour eviter collision Textual) ──

    def _on_mount_done(self, ticket, dataset):
        """Callback montage — PAS le lifecycle Textual on_mount()."""
    ...
    def _on_umount_done(self, ticket, dataset):
        if ticket.status == "completed":
            for e in self._entries:
                if e["dataset"] == dataset:
                    e["mounted"] = False
    ...
    def _on_verify_done(self, ticket, dataset):
        if ticket.status == "completed":
            for e in self._entries:
                if e["dataset"] == dataset:
                    e["verified"] = True

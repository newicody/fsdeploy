...
See docs/bridge-ui-scheduler.md for how the UI interacts with the scheduler.
"""
import os
import sys
...
class GraphViewScreen(Screen):
    BINDINGS = [
        ...
        Binding("h", "show_history", "Historique logs", show=True),
    ]
    DEFAULT_CSS = """
    ...
    #live-graph { height: 12; border: solid $accent; background: $surface; }
    ...
    @property
    def bridge(self):
        """Retourne l'instance du SchedulerBridge attachée à l'application.

        Utilisez‑la pour soumettre des intents ou interroger l'état du scheduler.
        Voir docs/bridge‑ui‑scheduler.md pour des exemples complets.
        """
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label("Graphe temps réel", classes="section-title")
        yield LiveGraph(id="live-graph")
        yield Rule()
    ...
    def _get_state(self) -> dict:
        # Try to get real state from the scheduler bridge.
        # See docs/bridge-ui-scheduler.md for details.
        if self.bridge and hasattr(self.bridge, "get_scheduler_state"):
            try:
                ...
                sys.stderr.write(
                    f"[GraphViewScreen] get_scheduler_state error: {e}\n"
                )
        # Fallback to generated data for development/demo
        import random
    ...
    def action_show_history(self) -> None:
        """Pousse l'écran d'historique des logs."""

...
from fsdeploy.lib.scheduler.model.event import (
...
class SocketSource(EventSource):
    ...
    Le chemin du socket est determine par le daemon via
    _resolve_socket_path() — fallback automatique si /run
    n'est pas writable.
    """
    ...
    def __init__(self, event_queue, socket_path: str | None = None):
        ...
        # Chemin effectif apres resolution (peut changer si bind echoue)
        self.effective_path: str = self.socket_path

    def _run(self) -> None:
        sock_path = Path(self.socket_path)

        # Tenter de nettoyer un vieux socket
        try:
        ...
        try:
            ...
            # /run pas writable — fallback user-writable
            srv.close()
            fallback = Path(f"/tmp/fsdeploy-{os.getuid()}.sock")
            fallback.unlink(missing_ok=True)
            srv = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            try:
                srv.bind(str(fallback))
                sock_path = fallback
            except OSError:
                srv.close()
                return
        ...
        self.effective_path = str(sock_path)
...
# Bus de messages global pour la communication interne
from .event_bus import MessageBus, message_bus

...
# Scheduler performance tuning (optional)
# FSDEPLOY_MAX_PARALLEL=5
# FSDEPLOY_MAX_RETRIES=3
# FSDEPLOY_RETRY_BASE_DELAY=1.0
ENVEOF
srun chmod 640 "$ENV_FILE"
srun chown "${REAL_USER}:${FSDEPLOY_GROUP}" "$ENV_FILE"
...
if as_user "$VENV_DIR/bin/python3" -c "import sys; sys.path.insert(0, '${INSTALL_DIR}'); import fsdeploy.lib.scheduler.intentlog.codec, fsdeploy.lib.scheduler.intentlog.log, fsdeploy.lib.intents.log_intent" 2>/dev/null; then
    ok "Modules fsdeploy importables"; (( _ok++ )) || true
else
    warn "Impossible d'importer certains modules fsdeploy"
fi

if as_user "$VENV_DIR/bin/python3" -c "import pytest" 2>/dev/null; then
    ok "pytest présent dans venv"; (( _ok++ )) || true
else
    warn "pytest absent (nécessaire pour les tests)"
fi

if as_user "$VENV_DIR/bin/python3" -m fsdeploy --help 2>/dev/null | grep -q "usage:"; then
    ok "CLI fsdeploy fonctionnel"; (( _ok++ )) || true
else
    warn "CLI fsdeploy ne répond pas"
fi

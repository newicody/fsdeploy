...
# ─────────────────────────────────────────────────────────────────────────────
_is_live() {
    # Heuristiques cumulées : cmdline live, /run/live, fstab vide/minimal
    grep -qiE 'boot=live|live-media|casper|nopersistent' /proc/cmdline 2>/dev/null && return 0
    [[ -d /run/live ]]         && return 0
    [[ -f /etc/live/config ]]  && return 0
    # fstab quasi-vide (< 3 entrées non-commentées) → probablement live
    local entries
...
_pre_checks() {
    ...
    # Espace disque sur /opt (venv + dépôt ≈ 600 MB)
    local avail_mb
...
if [[ $UPDATE_MODE -eq 1 ]]; then
    ...
    # 3. Migration config : ajouter les nouvelles clés sans écraser les valeurs existantes
    #    On délègue à Python (FsDeployConfig._apply_defaults re-valide le configspec)
...
try:
    ...
    cfg._apply_defaults()   # remplit les clés manquantes avec les défauts
    cfg.save()
...
PYEOF
    ...
    # 4. Mettre à jour le fichier .env si INSTALL_DIR a changé
    _write_env_file() {
...
ENVEOF
    ...
    [[ -d "$INSTALL_DIR" ]] && _write_env_file && ok ".env mis à jour"
...
if [[ $IS_LIVE -eq 1 ]]; then
    # Sur live, on remplace complètement (propre, pas de résidu)
...
EOF
    ...
    # Sur système installé : on ne touche qu'à ce qui manque
    local_sources=/etc/apt/sources.list

    # Vérifier si contrib/non-free déjà présent pour trixie
    if ! grep -q "trixie main contrib non-free" "$local_sources" 2>/dev/null; then
        # Réécrire proprement si la ligne trixie existe sans contrib/non-free
        if grep -q "trixie" "$local_sources" 2>/dev/null; then
            ...
            # Ajouter une entrée trixie (système strange mais on gère)
...
# Préférence backports (priorité basse sauf demande explicite)
...
# zfs-dkms lance une compilation en arrière-plan — on attend qu'elle finisse
# avant de tenter modprobe zfs
# ─────────────────────────────────────────────────────────────────────────────
...
_wait_dkms() {
    ...
    # Si le module est déjà là (système installé avec ZFS), pas besoin d'attendre
    ...
    while true; do
        ...
        if echo "$status" | grep -q "built"; then
            # Compilé mais pas encore installé → patienter un peu
            :
        fi
...
# Charger le module (silencieux si déjà chargé)
...
# Pas de re-authentification pour les membres du groupe
...
if [[ -f "${INSTALL_DIR}/requirements.txt" ]]; then
    ...
    warn "requirements.txt absent — à installer manuellement"
fi

# Permissions venv
srun chown -R "${REAL_USER}:${FSDEPLOY_GROUP}" "$VENV_DIR"
srun chmod -R g+rX "$VENV_DIR"
info "Venv → ${REAL_USER}:${FSDEPLOY_GROUP}  g+rX"

# Fichier .env — variables consommées par le wrapper et par config.py
as_user bash -c "cat > '${ENV_FILE}'" << ENVEOF
# fsdeploy — variables d'environnement
# Généré par launch.sh — $(date '+%Y-%m-%d %H:%M:%S')
# Source automatiquement par le wrapper /usr/local/bin/fsdeploy
FSDEPLOY_USER="${REAL_USER}"
FSDEPLOY_GROUP="${FSDEPLOY_GROUP}"
FSDEPLOY_INSTALL_DIR="${INSTALL_DIR}"
FSDEPLOY_VENV="${VENV_DIR}"
...
'exécute en UTILISATEUR NORMAL — sudo uniquement pour les opérations
# qui l'exigent (apt, groupadd, chown, sudoers, dkms…).
#
# Usage :
#   bash launch.sh                          # premier bootstrap
